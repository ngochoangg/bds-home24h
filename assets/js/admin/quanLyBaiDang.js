...
jQuery(($) => {
  ...
  $(document).on("click", ".btn-table-edit", (e) => {
    ...
    loadDistrict(thisId, getDistrictByProvinceCode);
  ...
  //Get Districts first
  function loadDistrict(id, getAll) {
    $.ajax({
      url: `${gURL}/post/${id}`,
      method: "GET",
      headers: {
  ...
  function setModalData(data) {
    ...
    $("#inpModalNgayTao").val(
      `${data.ngayTao ? new Date(data.ngayTao).toLocaleString() : "N/A"}`
    );
    ...
    $("#inpModalNgayCapNhat").val(
      `${
        data.ngayCapNhat ? new Date(data.ngayCapNhat).toLocaleString() : "N/A"
      }`
    );
  }
  ...
  function deletePost(postId) {
    if (confirm("Xác nhận xóa bài đăng với ID: " + postId + " ?")) {
      $.ajax({
        ...
        success: () => {
          $("#toastDeletedText").html(
            `Bài đăng với ID: ${postId} đã được xóa thành công và không thể hoàn tác!`
          );
